...
async fn metrics_route_handler() -> String {
    let data: TypesenseMetrics = get_typesense_metrics().await.unwrap();
    // format!("get_typesense_metrics, {:?}", data);

    let data2: TypesenseStats = get_typesense_stats().await.unwrap();
    format!(
